...
test('application', async () => {
  ...
  await userEvent.default.type(screen.getByLabelText(/new task name/i), task1);
  expect(screen.getByLabelText(/new task name/i)).toHaveDisplayValue(task1);
  await userEvent.default.click(screen.getByRole('button', { name: /add task/i }));
  ...
  await userEvent.default.clear(screen.getByLabelText(/new task name/i));
  await userEvent.default.type(screen.getByLabelText(/new task name/i), task2);
  ...
  const list1 = 'Random';
  await userEvent.default.clear(screen.getByLabelText(/new task name/i));
  await userEvent.default.type(screen.getByRole('textbox', { name: /new list name/i }), list1);
  expect(screen.getByRole('textbox', { name: /new list name/i })).toHaveDisplayValue(list1);
  await userEvent.default.click(screen.getByRole('button', { name: /add list/i }));
  expect(screen.getByRole('textbox', { name: /new list name/i })).toHaveDisplayValue('');

  await userEvent.default.clear(screen.getByRole('textbox', { name: /new list name/i }));
  await userEvent.default.type(screen.getByRole('textbox', { name: /new list name/i }), list1);
  ...
  await userEvent.default.click(screen.getByText(list1));
  ...
  await userEvent.default.clear(screen.getByLabelText(/new task name/i));
  await userEvent.default.type(screen.getByLabelText(/new task name/i), task3);
  await userEvent.default.click(screen.getByRole('button', { name: /add task/i }));

  expect(document.querySelector('[data-container="tasks"]')).toContainElement(screen.getByText(task3));

  await userEvent.default.click(screen.getByRole('link', { name: /general/i }));
